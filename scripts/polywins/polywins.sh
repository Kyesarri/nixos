# ./nixos/scripts/polywins/polywins.sh
# POLYWINS
...
generate_window_list() {
	...
	while IFS="[ .\.]" read -r wid ws cname cls host title; do
		...
		printf "%s" "%{A1:$on_click raise_or_minimize $wid:}" # a1 = left click
#		printf "%s" "%{A2:$on_click close $wid:}" # a2 = middle click
		printf "%s" "%{A3:$on_click close $wid:}" # a3 = right click
		printf "%s" "%{A4:$on_click increment_size $wid:}" # a4 = scroll up?
		printf "%s" "%{A5:$on_click decrement_size $wid:}" # a5 = scroll down?
