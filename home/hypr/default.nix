...
with lib; let
  cfg = config.services.hypr;
in {
  options.services.hypr = {
    enable = mkEnableOption "enable hyprland"; # will be services.hypr.enable = true; in host.nix
    animations = mkOption {
      type = types.str; # string, makes sense
      default = "default"; # default animations are planned to be flat, boring and vanilla just like me
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    home-manager.users.${user} = {
      ...
      # hyprland.conf, looking to mkoptions for animations, add default which is generated with
      # a option else? if a flag = false or string type is default / none
      # would be able to possibly change on a whim to a battery mode / idgaf mode
